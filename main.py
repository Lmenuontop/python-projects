...
while True:
    ...
    if (
        x_position + 32 > random_x_position
        and x_position < random_x_position + 32
        and y_position + 32 > random_y_position
        and y_position < random_y_position + 32
    ):
        random_x_position = random.randint(0, 1000)
        random_y_position = random.randint(-64, -25)
        score = 0
    ...
    if keys[pygame.K_KP3] and dev_mode == "y":
        ...
        y_position = 0
    # debug
    if keys[pygame.K_KP4] and dev_mode == "y":
        score = score + 1
        print("(cheated) score is, ", score)
    if keys[pygame.K_SPACE] and not bullet_fired:
        bullet_fired = True
        bulletxpos = (
            x_position  # Set bullet's x position to player's current x position
        )
    ...
    if bullet_fired:
        ...
        if (
            bulletxpos + 32 > random_x_position
            and bulletxpos < random_x_position + 32
            and bulletypos + 32 > random_y_position
            and bulletypos < random_y_position + 32
        ):
            score += 1
            random_x_position = random.randint(0, 1000)
            random_y_position = random.randint(-64, -25)
